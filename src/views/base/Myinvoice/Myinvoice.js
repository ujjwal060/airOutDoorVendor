...
const VendorPayoutTable = () => {
  ...
  const fetchPayoutData = async () => {
    ...
    try {
      const response = await axios.get(`http://18.209.197.35:8000/payouts/getVendorPay/${vendorId}`);
  ...
  const handleCashoutRequest = async () => {
    ...
    try {
      const response = await axios.post('http://18.209.197.35:8000/payouts/cashoutRequest', {
        vendorId,
