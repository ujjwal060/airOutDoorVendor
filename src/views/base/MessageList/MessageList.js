...
const PropertyManagement = () => {
  ...
  const handleDeleteProperty = async (id) => {
    try {
      await axios.delete(`http://44.196.192.232:8000/host/deleteproperty/${id}`);
      fetchProperties();
    } catch (error) {
  ...
  const handleSave = async () => {
    try {
      if (currentProperty._id) {
        ...
        await axios.put(`http://44.196.192.232:8000/host/updateproperty/${currentProperty._id}`, currentProperty);
      } else {
        // Add new property
        await axios.post('http://44.196.192.232:8000/host/add', currentProperty);
      }
      fetchProperties();
